...
function check_install(){
    thecmd="$1"
    the_installcmd="$2"
    if ! command -v $thecmd 2>&1 >/dev/null
    then
...
check_install "brew" '/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install.sh)" </dev/null ## /dev/null skips pressing enter for the installation'
